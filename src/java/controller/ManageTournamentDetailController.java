...
public class ManageTournamentDetailController extends HttpServlet {
    ...
    private final String SUCCESS = "manageTournamentDetail.jsp";
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            try {
                ...
                if (tour != null) {
                    ...
                    int num = rdao.getNumberRegistered(0, tournamentID);
                    request.setAttribute("numberOfPlayer", num);
                    if (tour.getTournamentStatus() == 0) {
                        ...
                        if (rounds.isEmpty()) {
                            RoundDTO roud = null;
                            int numberOfPlayer = rdao.getNumberRegistered(1, tournamentID);
                            if (numberOfPlayer >= 40) {
                                String[] names = {"Qualified", "Top30", "Top20", "Top10", "Top4"};
                                for (String name : names) {
                                    roud = new RoundDTO();
                                    tour = new TournamentDTO();
                                    tour.setTournamentID(tournamentID);
                            ...
                            rounds = roudao.getAllByTID(tournamentID);
                            request.setAttribute("rounds", rounds);
                            url = SUCCESS;
                        } else {
                            request.setAttribute("rounds", rounds);
